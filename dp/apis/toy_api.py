...
class ToyAPI(API):
    ...
    def _variation(self, samples: np.ndarray, size: str, additional_info: np.ndarray, variation_degree: Union[np.ndarray, float], demo_samples: Optional[np.ndarray] = None, sample_weight: float = 1.0):
        sample_variate = np.zeros_like(samples)
        demo_variate = np.zeros_like(samples)
        ...
        if sample_weight > 0:
            ...
            noises = np.array([self.rng.normal(0, (1 + deg), 2) for deg in degrees])
            sample_variate = samples + noises
        if sample_weight < 1:
            # demonstration-based
